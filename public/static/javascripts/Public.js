...
// -------------------------登录页面---------------------------------------------------
	...
	$('#login').click(function(event) {
		...
		var avatar = $('.login img').attr('src'); // 用户头像id
...
// -----下边的代码不用管---------------------------------------
	...
	$('.clapboard').click(function(event) {
		...
		$('.user_portrait img').attr('src', '/public/static/images/user/' + $('.user_portrait img').attr('ptimg') + '.png');
	...
	$('.select_portrait img').hover(function() {
		...
		$('.user_portrait img').attr('src', '/public/static/images/user/' + portrait_id + '.png');
		$('.login input').val(userName); 
	}, 
	function() {
		var t_id = $('.user_portrait img').attr('portrait_id');
		$('.user_portrait img').attr('src', '/public/static/images/user/' + t_id + '.png');
	});
	...
	function sends_message (userName='', avatar='', message='',time='') {
		if(message!='') {
			$('.main .chat_info').html($('.main .chat_info').html() + '<li class="right"><img src="' + avatar +' " alt=""><b>' + userName + '</b><i>'+time+'</i><div class="aaa">' + message  +'</div></li>');
		}
		// 滚动条滚到最下面
		$('.scrollbar-macosx.scroll-content.scroll-scrolly_visible').animate({
			scrollTop: $('.scrollbar-macosx.scroll-content.scroll-scrolly_visible').prop('scrollHeight')
